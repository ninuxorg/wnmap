function DistanceCalculator (node)
{
	...
	this.getContent = function()
	{
		...
		activegroup.label = "Nodi attivi"

		var inactivegroup = document.createElement("optgroup");
		inactivegroup.label = "Nodi potenziali";

		var markergroup = document.createElement("optgroup");
		markergroup.label = "I miei segnaposto";
		...
		text.innerHTML = "Calcola la distanza da <b>" + this.node.name + "</b> a ";
			...
			for (key in markers) {
				...
				if (otherMarker.name == select.value) {
					...
					if (km > 2) {
						...
						result.innerHTML = roundNumber(meters,2) + " metri";
					}

					result.innerHTML += "<br/>";

					if (miles > 1) {
						result.innerHTML += roundNumber(miles,2) + " miglia";
					} else {
						result.innerHTML += roundNumber(yards,2) + " iarde";
					}
